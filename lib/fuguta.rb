...
class Fuguta
  ...
  def self.walk_tree(conf, &blk)
    raise ArgumentError, "conf must be a Configuration. Got '#{conf.class}' instead." unless conf.is_a?(Configuration)
  ...
  module ConfigurationMethods
    module ClassMethods
      ...
      # # Example:
      # class Base
      #   include Fuguta::ConfigurationMethods
      ...
      def def_configuration(&blk)
        ...
        if self.const_defined?(:Configuration, false)
          unless self.const_get(:Configuration, false) < Fuguta
            raise TypeError, "#{self}::Configuration constant is defined already for another purpose."
          end
        else
          self.const_set(:Configuration, Class.new(self.configuration_class || Fuguta))
  ...
  def initialize(parent=nil)
    unless parent.nil?
      raise ArgumentError, "#{parent.class}" unless parent.is_a?(Fuguta)
    end
    @config = {}
    @parent = parent
